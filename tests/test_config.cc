...
sylar::ConfigVar<std::map<std::string,int>>::ptr g_str_int_map_value_config =
    sylar::Config::Lookup("system.str_int_map", std::map<std::string,int>{{"k",1}}, "system str int map");

sylar::ConfigVar<std::unordered_map<std::string,int>>::ptr g_str_int_umap_value_config =
    sylar::Config::Lookup("system.str_int_umap", std::unordered_map<std::string,int>{{"k",1}}, "system str int umap");


void print_yaml(const YAML::Node& node,int level){
    if(node.IsScalar()){
        SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << std::string(level + 4 , ' ') 
            << node.IsScalar() << " - " << node.Type() << " - " << level;
    }else if (node.IsNull()){
        SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << std::string(level + 4 , ' ') 
...
void test_yaml() {
    YAML::Node  root = YAML::LoadFile("/root/sylar-project/sylar/bin/config/test.yml");
...
void test_log() {
    ...
    YAML::Node root = YAML::LoadFile("/root/sylar-project/sylar/bin/config/log.yml");
    sylar::Config::LoadFromYaml(root);
    std::cout << "====================" << std::endl;
    std::cout << sylar::LoggerMgr::GetInstance()->toYamlString() << std::endl;
    std::cout << "====================" << std::endl;
